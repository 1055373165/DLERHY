"""Structure Agent (advisory): a model looks at suspicious PDF pages and files structure issues.
...
It does not edit structure. Block-level parse forks are not built yet, so
suggestions are non-blocking issues for a person (or a later recovery-skill
change plus structure refresh) to act on. See docs/agent-upgrade/02 §H3.
"""
...
def structure_tool_registry() -> ToolRegistry:
    return ToolRegistry(
        [
...
class StructureAgent:
    ...
    def registry() -> ToolRegistry:
        return structure_tool_registry()
