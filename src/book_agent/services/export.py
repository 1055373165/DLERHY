...
class ExportService:
    ...
    def _build_merged_document_html(
        ...
        return (
            ...
            ".sidebar{position:sticky;top:18px;align-self:start;background:rgba(255,253,248,.82);backdrop-filter:blur(10px);border:1px solid rgba(213,199,179,.85);border-radius:22px;padding:22px 18px;box-shadow:var(--shadow);}"
            ...
            ".toc-item a{display:block;padding:10px 12px;border-radius:12px;color:var(--ink);background:rgba(217,238,240,.45);border:1px solid transparent;font-family:var(--font-ui);font-size:14px;line-height:1.4;}"
        ...
        for block in bundle.blocks:
            ...
            render_blocks.append(
                MergedRenderBlock(
                    ...
                    title=(bundle.chapter.title_src if block.block_type == BlockType.HEADING else None),
                    source_text=block.source_text,
                    target_text=target_text or None,
                    source_metadata=source_metadata,
                    source_sentence_ids=[sentence.id for sentence in block_sentences],
    ...
    def _render_mode_for_block(self, block, block_sentences: list[object], source_metadata: dict[str, object]) -> str:
        ...
        if self._looks_like_reference_literal(block.source_text):
    ...
    def _artifact_kind_for_block(self, block, render_mode: str) -> str | None:
        tag = (block.source_span_json or {}).get("tag")
    ...
    def _export_epub_assets_for_document_bundle(
        ...
        return self._export_epub_assets(
    ...
    def _export_epub_assets(
        ...
        if source_type != SourceType.EPUB or not source_path:
            return {}
