...
def _detect_paragraph_break_positions(raw: str) -> list[int]:
    """Return character positions in ``raw`` where a paragraph starts.

    Uses the same column-wrap heuristic as ``_reflow_pdf_source``: a
    ``\\n`` is a real paragraph break only if the preceding line is
    markedly shorter than the typical column width AND ends with a
    sentence terminator. Blank lines (``\\n\\n``) are always paragraph
    breaks.

    Always returns ``[0, ...]`` — position 0 is always a paragraph
    start. Subsequent positions mark the first character of paragraphs
    ...
    breaks = [0]
    cursor = 0  # position of the start of the current line
    for i in range(len(lines) - 1):
        ln = lines[i]
        cursor_after = cursor + len(ln) + 1  # +1 for the \n separator
        # Blank line → next non-blank line starts a new paragraph.
        if ln.strip() == "":
            if cursor_after < len(raw):
                breaks.append(cursor_after)
        elif (
            len(ln.rstrip()) < short_threshold
            and _PARAGRAPH_END_RE.search(ln.rstrip())
        ):
            if cursor_after < len(raw):
                breaks.append(cursor_after)
        cursor = cursor_after
...
def _block_zh_chunks_grouped(
    session, block
) -> tuple[list[list[str]], list[str]]:
    """Return chunks grouped by source paragraph.

    Outer list = paragraphs (in source order). Inner list = zh
    sentence chunks belonging to that paragraph. Second tuple is the
    flat list of untranslated source texts (sentences that have no
    target segment or whose translation was stub-filtered).
    """
    ...
    # Per-sentence record so we can sort within paragraphs by ts_ordinal.
    per_sentence: list[tuple[int, int, str]] = []  # (para_idx, ts_ordinal, chunk)
    untranslated: list[str] = []
    search_cursor = 0

    for sentence_id, _, source_text in sentence_rows:
        # Find this sentence's start position in the raw block source.
        offset = _find_sentence_offset(source_text, raw, search_cursor)
        if offset >= 0:
            search_cursor = offset + 1
        else:
            offset = search_cursor
        para_idx = _paragraph_index_for_offset(offset, breaks)
        ...
        if _is_stub_translation(text_zh):
            untranslated.append(source_text)
            continue
        per_sentence.append((para_idx, int(ts_ordinal or 0), _strip_typeset_codes(text_zh)))

    # Group per_sentence into paragraph buckets (preserve sentence
    # ordering within a paragraph by ts_ordinal).
    grouped: list[list[str]] = [[] for _ in range(num_paragraphs)]
    for para_idx, _ord, chunk in per_sentence:
        if 0 <= para_idx < num_paragraphs:
            grouped[para_idx].append(chunk)
        else:
            grouped[-1].append(chunk)
    # Sort within each paragraph by ts_ordinal — collect tuples first.
    by_para: dict[int, list[tuple[int, str]]] = {}
    for para_idx, ord_, chunk in per_sentence:
        by_para.setdefault(para_idx, []).append((ord_, chunk))
    grouped = []
    for i in range(num_paragraphs):
        bucket = sorted(by_para.get(i, []), key=lambda kv: kv[0])
        grouped.append([c for _, c in bucket])
    # Drop trailing empty paragraphs (e.g. when last detected paragraph
    # had no translatable sentences).
    while grouped and not grouped[-1]:
        grouped.pop()
    return grouped, untranslated


def _find_sentence_offset(sent_text: str, raw: str, start_at: int) -> int:
    """Locate ``sent_text``'s first occurrence in ``raw`` starting at
    ``start_at``. Tolerates collapsed whitespace and PDF line-wraps in
    the raw block by matching on the sentence's first few WORDS."""
    if not sent_text or not raw:
        return -1
    tokens = sent_text.split()
    if not tokens:
        return -1
    # Use the first 3 words (or all if shorter) as the probe.
    probe = " ".join(tokens[:3])
    # Try direct substring first.
    pos = raw.find(probe, start_at)
    if pos >= 0:
        return pos
    # Fall back to a regex with flexible whitespace.
    pattern = re.compile(
        r"\s+".join(re.escape(tok) for tok in tokens[:3]),
        flags=re.DOTALL,
    )
    m = pattern.search(raw, start_at)
    if m:
        return m.start()
    return -1
...
def _block_zh_chunks(session, block) -> tuple[list[str], list[str]]:
    """Return (flat translated chunks in source order, untranslated).

    Wraps :func:`_block_zh_chunks_grouped` for callers that don't care
    about source-paragraph boundaries.
    """
    grouped, untranslated = _block_zh_chunks_grouped(session, block)
    flat = [chunk for group in grouped for chunk in group]
    return flat, untranslated
...
def _paragraph_html_multi(groups: list[list[str]]) -> str:
    """Render multi-paragraph chunks as a sequence of ``<p>`` elements.

    Used when one source block contains MULTIPLE PDF paragraphs (the
    parser stored them in a single block joined by ``\\n``). The
    paragraph-detection layer assigns each translated sentence chunk
    to its source paragraph; we then emit one ``<p>`` per group so
    Chinese paragraph structure mirrors the English source.
    """
    parts = []
    for group in groups:
        if group:
            html_part = _paragraph_html(group)
            if html_part:
...
# Page-footer / page-number artifacts that the parser leaks into body
# blocks. Examples seen in production:
#   "GE 121 9312"   chapter-end Manning typesetter ID
#   "4"             bare page number on its own line
#   "Chapter 3"     mid-text chapter label that escaped the heading filter
# Short, low-information, almost certainly NOT chapter content.
...
# meaningless single-token line (page number "5", broken header):
#   "这是当前段落中唯一的句子。"  /  "当前段落仅此一句。"
...
def _is_stub_translation(zh_joined: str) -> bool:
    ...
    if not s or len(s) > 60:
        return False
    return any(needle in s for needle in _STUB_TRANSLATION_NEEDLES)


# Manning typesetter codes / running-page IDs ("JL 545 2230", "GE 121 9312")
# embedded inside otherwise-translated paragraphs. The parser cannot remove
# them because they sit on the same line as body text in the PDF; the model
# faithfully copies them through. Strip them post-hoc so the rendered Chinese
# reads cleanly. Pattern: 2 ASCII letters + ≥2 number groups, surrounded by
# any whitespace / Chinese parens.
_INLINE_TYPESET_CODE = re.compile(
    # No leading \b: in mixed CJK + ASCII text Python's word-boundary rule
    # treats the gap between "如" and "JL" as inside-word, so \b fails.
    r"(?:[（(]\s*(?:如|示例|例如)?\s*)?"
    r"(?<![A-Za-z])[A-Z]{2}\s+\d{2,4}(?:\s+\d{2,4}){1,2}(?![A-Za-z\d])"
    r"(?:\s*[)）])?"
)


def _strip_typeset_codes(text: str) -> str:
    if not text:
        return text
    cleaned = _INLINE_TYPESET_CODE.sub("", text)
    # Tidy doubled spaces / dangling separators left behind.
...
def main() -> int:
    ...
    repair_stats: dict[str, int] = {
        ...
        "page_artifacts_suppressed": 0,
        "stub_translations_suppressed": 0,
    }
    ...
    with session_scope(factory) as session:
        ...
        for block in blocks:
            ...
            if block.id in skip_render_block_ids:
                continue
            # Skip page-footer / page-number artifacts the parser leaked
            ...
            # already rendered as <h2> earlier in the chapter, and the
            # trailing page number is meaningless to readers.
            if btype == "footnote":
                repair_stats["page_artifacts_suppressed"] += 1
                continue
            ...
            if block.id in listing_header_ids:
                ...
                continue
            chunks, untranslated = _block_zh_chunks(session, block)
            # Stub-translation guard: the translator sometimes turns a
            # 1-token source (e.g. "5") into "这是当前段落中唯一的句子。"
            # — short, formulaic, and not actual content. Drop those when
            # the source is also tiny (≤4 chars), so we don't accidentally
            # discard a legitimately short translated paragraph.
            src_clean = (block.source_text or "").strip()
            if (
                btype in {"paragraph", "heading"}
                and chunks
                and len(src_clean) <= 4
                and _is_stub_translation("".join(chunks))
            ):
                repair_stats["stub_translations_suppressed"] += 1
                continue
            ...
            if btype == "heading" and not chunks:
                ...
                if _m:
                    ...
                    if fallback_zh:
                        chunks = [fallback_zh]
                        untranslated = []
            # Multi-paragraph rendering: ordinary body blocks whose
            # source contained MULTIPLE PDF paragraphs (joined by `\n`
            # inside one block) should render as multiple ``<p>`` tags
            # — not one mega-paragraph. We only apply this when no
            # cross-block transformation modified the chunk list above
            # (callout glue, join_chain merge, etc.) — those produce
            ...
            cross_block_modified = (
                ...
                or block.id in ordered_list_head_to_items
                or block.id in listing_header_ids
            )
            if (
                btype == "paragraph"
                and chunks
                and not cross_block_modified
            ):
                grouped, _u = _block_zh_chunks_grouped(session, block)
