...
def _block_zh_chunks(session, block) -> tuple[list[str], list[str]]:
    ...
    out_chunks = [chunk for _, chunk in rendered]
    # Stub-translation guard: drop chunks that are the translator's
    # generic "this is the only sentence in this paragraph" template
    # (it surfaces when the source is a 1-token line or a heading the
    # model couldn't parse). Treat those sentences as untranslated so
    # the renderer falls back to the English source — much better than
    # publishing an autogenerated stub as if it were the translation.
    cleaned_chunks: list[str] = []
    for chunk in out_chunks:
        if _is_stub_translation(chunk):
            untranslated.append(block.source_text or "")
            continue
        # Strip Manning typesetter codes ("JL 545 2230") that the model
        # copied straight through from the running-header text into the
        # body translation.
        cleaned_chunks.append(_strip_typeset_codes(chunk))
    return cleaned_chunks, untranslated
