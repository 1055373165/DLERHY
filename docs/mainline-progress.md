...
- 当前真正缺的是“incident 自动生成可执行修复计划，并为后续 repair agent/repair lane 提供确定 handoff”。
...
- 把 runtime incident 补成明确的 `repair plan / handoff` 能力，避免修复动作散落在 controller 内部
- 把 repair plan 继续推进成 runtime-owned repair dispatch surface
- 把后续 patch validation、bundle publish/rollback、replay scope 统一绑定到这条 repair plan lineage
