...
- 当前真正缺的是“把这条 repair-agent adapter contract 继续接到真正独立的 remote / agent-backed executor”，让 runtime 不只是按 hint/version 选到不同本地 adapter，而是能把 repair lane 交给不同 repair executors。
...
- 把 repair-agent adapter contract 继续推进到 remote / agent-backed repair executors
