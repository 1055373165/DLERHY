...
- [ ] **统一 LLM client**（`workers/providers`）：httpx + 连接池 + 整体截止时间 + 指数退避（jitter、上限、遵守 Retry-After）；Responses/Chat 两种模式；修缓存 token 字段（R12）；每次调用必发 `llm.call.started/completed/failed`，含 `call_kind`（translate / concept / glossary / survey / review / repair / qa）、`run_id`、`agent_id`、`cost_usd`。概念解析、术语抽取、一致性 pass 全部改走它。
- [ ] **预算与成本单一口径**：`usage_summary` 从 events 派生（或事件写入时同步累加）；预算护栏在 work 线程完成时也检查；物化视图改为触发器/后台刷新（R15）。
...
- [ ] **Terminology Agent**：在 translate 阶段前作为必需阶段（`run_plan` 新增 `terminology` stage）；输入全书抽样 + 现有 `GlossaryExtractionService` 结果；输出 BOOK.md 术语段与 `term_entries`（PREFERRED），需要 LOCKED 的走审批（自动策略：人名/缩写/出现 ≥ N 次自动锁）。
...
- [ ] **Structure Agent**：只对 `parse_confidence` 低 / `layout_risk` 高 / sanity 失败的页触发；工具 `render_page_image`（多模态读页）、`split_block`、`merge_blocks`、`relabel_block`、`link_caption`；每个结构改动产生新 parse revision 并迁移句子（先落地「句子退役模型」——这是 PLAN P3.3 的未完成项，也是锚点方案立项的前提）。
...
H3 的 Structure Agent 依赖「句子退役模型 + 稳定锚点」的数据迁移决策，应在 H1 期间完成设计评审。
