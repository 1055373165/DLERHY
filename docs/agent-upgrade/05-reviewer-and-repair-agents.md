...
> 状态：H2 实施稿（2026-09-17）。Reviewer Agent 已实现（`harness/agents/reviewer.py`，测试 `tests/test_reviewer_agent.py`）；Repair Agent 已实现（`harness/agents/repair.py`、`services/segment_edit.py`，测试 `tests/test_repair_agent.py`、`tests/test_segment_edit.py`），默认关闭。前置已完成：输出 guardrail（同 turn 修复）、issue 账本（`review_issue_events`、人工决定优先、action 不再每轮重置）、issue API。本篇固定两个 agent 与现有规则审校、修复循环的接口，避免「模型 issue」与「规则 issue」各走一套。
