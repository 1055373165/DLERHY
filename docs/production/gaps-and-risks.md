...
| 2026-09-18 | R22（多实例部分） | 见 H4 多实例提交 | 迁移 0040：`document_runs.executor_owner / executor_lease_expires_at`，supervisor 与 run loop 每次 tick 续约，其他实例在租约过期后接管，`stop()` 释放；`BOOK_AGENT_RUN_EXECUTOR_ENABLED=false` 部署纯 API 副本；work item 候选查询 `FOR UPDATE SKIP LOCKED`（CAS 仍是正确性保证）；`provider_credentials.config_revision` 使 worker 缓存跨进程失效（最多每 2 秒读一次库）；alembic 在 PostgreSQL 上持 `pg_advisory_xact_lock` 迁移；worker_instance_id 带 host:pid。未做：`wake()` 仍只唤醒本进程（其他实例靠轮询，最长一个 poll 间隔） |
| 2026-09-18 | R1（后续） | 见 H4 OIDC 与按 org 凭据/预算提交 | OIDC bearer JWT（JWKS 缓存、未知 kid 限速重载、只收非对称算法、org/角色声明映射）；provider 凭据按 org 作用域（共享 + 各 org 覆盖，跨作用域 404）；org 月度预算（启动/恢复 409，运行中暂停）。未做：非 run 调用的花费归属、前端预算展示 |
| 2026-09-18 | 付费实跑发现 | 见 H4 稳健性提交 | 思考模型把输出预算用尽 → 截断被单独建模并暂停 run；失败调用的 token 进入账本（回滚后补写）；work item 默认 6 次尝试 + 指数退避；审批恢复补跑同一步剩余调用、一步的审批一次性提交；顾问型 agent 失败降级不再拖垮整个 run |
