...
围绕机器学习（ML）、深度学习（DL）和人工智能（AI）等术语的炒作已创历史新高。公众最初接触这些术语，很大程度上得益于一款名为ChatGPT的产品，它是OpenAI公司构建的一种生成式AI。如今，我们看到生成式AI产品层出不穷：谷歌的Gemini、微软的Copilot、Meta的Llama、Anthropic的Claude，以及新秀DeepSeek，每日新闻中随处可见。仿佛一夜之间，计算机在对话、学习和执行复杂任务方面的能力实现了飞跃。新生的生成式AI公司不断涌现，现有企业也公开投入数十亿美元到这一领域。该领域的技术正以令人瞠目的速度演进。

本书旨在通过揭示ChatGPT及相关技术背后的奥秘，帮助你理解这个新世界。我们将涵盖理解其内部工作原理所需的知识，以及各个组件（数据和算法）如何堆叠在一起，创造出我们使用的工具。我们还将讨论各种场景，其中这项技术可以构成更广泛系统的基石，以及另一些场景中基于大语言模型（LLM）的系统可能并非明智之选。阅读本书后，你将理解像ChatGPT这样的生成式AI究竟是什么、它能做什么和不能做什么，以及更重要的是，其局限性背后的“为什么”。有了这些知识，你将能更有效地使用这一技术家族，无论你是用户、软件开发者，还是在组织中决定是否以及如何将其纳入产品或运营的商业决策者。这一基础还将为你深入学习该领域提供跳板，让你具备理解深度研究和其他著作所需的知识。

### 1.1 生成式AI的语境

首先，在谈论LLM、GPT以及依赖它们的各种工具时，我们需要更具体地明确我们讨论的内容。ChatGPT中的GPT代表生成式预训练变换器（Generative Pretrained Transformer）。每个单词在ChatGPT的语境中都具有特定的含义。我们将在后续章节专门讨论“预训练”和“变换器”的含义，但这里我们首先讨论“生成式”在此语境中的含义。像ChatGPT这样的AI聊天机器人是生成式AI的一种形式。广义上，生成式AI是一种能够基于过去观察到的数据并受人们认为愉悦和准确输出的影响，创建或生成各种媒体（例如文本、图像、音频和视频）的软件。例如，如果向ChatGPT提示“写一首关于雪落在松树上的俳句”，它将利用其训练过的所有关于俳句、雪、松树和其他诗歌形式的数据，生成一首新的俳句，如图1.1所示。

![图 1.1 ChatGPT生成的简单俳句](assets/fig-1-1-ecb14124ce.png)

*图：图1.1 ChatGPT生成的简单俳句*

从根本上说，这些系统是生成新输出的机器学习模型，因此生成式AI是一个恰当的描述。图1.2展示了一些可能的输入和输出。虽然ChatGPT主要处理文本输入和输出，但它也更实验性地支持其他数据类型，如音频和图像。然而，根据我们的定义，你可以想象许多不同类型的算法和任务都属于生成式AI的范畴。

![图 1.2 生成式AI接收一些输入（数字、文本、图像）并产生新的输出（通常是文本或图像）。任何输入或输出的组合都是可能的，输出的性质取决于算法训练的目标。它可以是添加细节、重写为更短、外推缺失部分等。](assets/fig-1-2-796f14358b.jpg)

*图：图1.2
生成式AI接收一些输入（数字、文本、图像）并产生新的输出（通常是文本或图像）。任何输入或输出的组合都是可能的，输出的性质取决于算法训练的目标。它可以是添加细节、重写为更短、外推缺失部分等。*

![图 1.3 一张高层次示意图，展示了您将熟悉的各类术语及其关联。生成式AI是对功能的描述：即生成内容的功能，并采用AI技术实现该目标。](assets/fig-1-3-a353ac77c0.png)

*图：图1.3 一张高层次示意图，展示了您将熟悉的各类术语及其关联。生成式AI是对功能的描述：即生成内容的功能，并采用AI技术实现该目标。*

注意：视觉和语言并非生成式AI的唯一选项。音频生成（想想文本转语音，例如GPS报路名）、下棋（如国际象棋），甚至蛋白质折叠都使用了生成式AI。本书将主要聚焦于文本和语言，因为它们是GPT和LLM使用的主要数据类型。

顾名思义，这些模型并不小。具体来说，据传ChatGPT包含1.76万亿个参数，这些参数决定了它的行为方式。每个参数通常以浮点数（带小数点的数）形式存储，占用4字节的存储空间。这意味着该模型本身需要7TB的内存来存储。这个尺寸超过了大多数人的计算机内存容量，更不用说在拥有80GB内存的最强图形处理单元（GPU）中了。GPU是专用硬件组件，擅长执行使LLM成为可能的数学运算。目前，构建LLM需要许多GPU，因此我们已经涉及到大量的计算基础设施和跨多台机器的复杂性。相比之下，更普通的语言模型在大多数情况下只有2GB或更小——小5000多倍，在更标准的硬件上构建和使用这样的模型时，这是一个更合理的尺寸。

许多研究人员正在探索如何让LLM消耗更少的内存。有时，这些技术包括使用“混合精度”[2]方法，将参数存储所需字节数降至4字节以下。该方法用2字节或更少来存储部分LLM参数，在精度和内存效率之间做出权衡。最终，对精度的影响通常可以忽略不计。这种优化是研究人员为使LLM更高效利用资源而采取的众多措施之一。

GPU替代方案：虽然GPU是目前训练LLM最常用的硬件，但它们并非唯一选择。越来越多的公司正在开发专用硬件，这些硬件为训练机器学习模型提供了通用优势。例如，2018年，谷歌将其张量处理单元（TPU）[3]作为谷歌云平台（GCP）的一部分向公众开放使用。虽然TPU的计算能力通常低于GPU，但其专用架构使它们在特定机器学习任务上表现优于GPU。

### 1.2 你将学到什么

在本书中，我们将解释大语言模型的工作原理，并为您提供理解它们所需的词汇。阅读完成后，您将对大语言模型是什么及其运行涉及的关键步骤有一个对话级的理解。此外，您还将了解大语言模型合理能力范围，特别是与部署或使用相关的考量。我们将讨论大语言模型的根本局限性，并提供如何规避这些局限性，或者何时应完全避免使用大语言模型及更广泛的生成式AI的建议。请记住，构建ChatGPT、Claude或Gemini时，Transformer组件的组合方式非常微妙，本书主要关注这些系统的共性。事实上，我们无法知晓这些大语言模型之间的一些实际差异，因为尽管商业大语言模型提供商分享了大量关于其模型的信息，但他们并未透露某些可能被视为商业机密的信息。鉴于基于Transformer的大语言模型将对世界产生的影响，本书特意面向广泛读者。未来几年，来自各种背景的程序员、高管、经理、销售人员、艺术家、作家、出版商等众多人群都将不得不与这些模型互动，或面临其带来的职业影响。因此，我们假设亲爱的读者您有基本的编程背景，熟悉编程的基本概念：逻辑、函数，甚至一些数据结构。此外，您不需要是数学家；我们会在必要之处展示一些数学知识，但这对于理解大语言模型的工作原理并非必需。这种定位意味着本书中会呈现很少的代码。如果您想直接深入构建和使用大语言模型，Manning出版社的其他书籍，如Sebastian Raschka的《从零构建大语言模型》（2024年）或Edward Raff的《深度学习内部原理》（2022年），将与本书内容互为补充。然而，如果您想理解为何您正在使用的大语言模型会产生异常输出、您的团队应如何利用大语言模型、或应避免在哪些场景使用大语言模型，又或者您的同事缺乏机器学习背景且需快速具备对话级能力，那么本书正是您和您的同事所需的。

具体来说，本书第一部分聚焦于大语言模型的功能：它们的输入与输出、如何将输入转化为输出，以及我们如何约束这些输出的性质。第二部分则关注人的行为：人们如何与技术交互，以及这给生成式AI的使用带来了哪些风险。类似地，我们还将讨论使用和构建大语言模型时出现的一些伦理问题。
...
生成式AI（GAI或GenAI）将改变我们生产和与信息交互的方式。2022年11月ChatGPT的推出凸显了现代AI的能力，并吸引了全球大部分人的关注。目前，你可以免费在https://chat.openai.com/注册并尝试使用。如果你输入文本提示“用两句话总结以下文本”，然后跟上本章所有的介绍性文字，你将得到类似如下的结果。“最近对人工智能，尤其是OpenAI的ChatGPT等大型语言模型（LLM）的关注激增，凸显了它们在自然语言处理方面的巨大能力。本书旨在为读者提供关于LLM的对话式理解，包括其操作细节、潜在应用、局限性以及使用时的伦理考量，同时假设读者仅具备基本的编程概念和极少的数学背景。”这相当令人印象深刻，对普通读者来说，这种能力似乎凭空出现。”当你访问OpenAI的网站并注册ChatGPT时，你可能会注意到一个类似图1.4所示的选项。顾名思义，GPT-4意味着OpenAI（截至撰写本文时）正在开发其第四代GPT模型。像GPT-4这样的LLM是机器学习研究中的一个成熟领域，旨在创建能够综合和处理信息并生成看似人类输出的算法。这种能力开启了人与机器之间先前只存在于科幻小说中的多个交互领域。ChatGPT中编码的语言表示的强大之处使其能够实现令人信服的对话、指令遵循、摘要生成、问答、内容创作等众多应用。事实上，这项技术的许多可能应用可能还不存在，因为

![图 1.4 当你注册OpenAI的ChatGPT时，有两个选项：可以免费使用的GPT-3.5模型，或者需要付费的GPT-4模型。](assets/fig-1-4-dfd099cc8e.png)

*图：图1.4 当你注册OpenAI的ChatGPT时，有两个选项：可以免费使用的GPT-3.5模型，或者需要付费的GPT-4模型。*

对读者您而言，关键点在于这项技术并非凭空出现，而是过去十年机器学习领域逐年显著进步、稳步发展的成果。因此，我们对大语言模型的工作原理及其可能出错的方式已经有相当深入的了解。我们假设读者只具备最基础的知识背景，这样您就可以把这本书推荐给朋友和家人。（其中一位作者希望这本书能送给他的母亲——母亲为他深感骄傲，尽管并不完全清楚他的工作具体是什么。）因此，在深入之前，我们需要先填补背景知识上可能存在的巨大鸿沟。第一章的目标就是为你提供这些背景，从而第二章就可以开始回答这个问题：计算机究竟是如何总结本书引言部分的？

### 1.4 到底什么是智能？

从市场营销角度看，人工智能是一个极佳的名称，尽管它最初是用来指代整个学术研究领域的。这种做法导致了一个微妙的问题，它让人们形成了对AI工作原理的错误认知模型。我们将努力避免强化这一模型。为了解释原因，我们将讨论为什么人工智能并非一个那么好的名称。我们可以通过思考一个简单的问题来轻松证明这一点：什么是智能？

你可能认为像智商（IQ）测试这样的东西能帮助我们回答这个问题。IQ测试与学业表现等诸多结果高度相关，但它们并未提供对智力的客观定义。研究表明，先天遗传和后天环境都在一定程度上影响一个人的IQ。我们也应对将智力简化成一个数字的做法持怀疑态度——毕竟，我们常批评一个人只是“书呆子气”而缺乏“街头智慧”。即使我们知道了什么是智力，又是什么让它成为“人工”的呢？智力难道是添加了人工香料和食用色素吗？关键在于，IQ测试衡量的是你在有限能力范围内的表现，主要是在时间限制下解决特定类型的逻辑谜题，但它们无助于我们理解智力的本质。事实是，人们对智力的理解并不完美。人工智能领域长期以来一直试图让计算机——这种刻板、确定、遵循规则的机器——执行那些人类能够完成但无法给出精确定义或指令的任务。例如，如果我们想让计算机数到1000并打印出所有能被5整除的数字，我们可以写出详细的指令，几乎任何程序员都能将其转换成代码。但如果我要求你编写一个程序来检测任意一张图片中是否有猫，那将是一个截然不同的挑战。你需要在某种程度上精确定义什么是猫，然后处理所有检测猫的细节。我们到底如何编写代码来找出并区分猫的胡须和狗的胡须？当猫没有胡须时，我们又如何成功识别它？归根结底，这并不容易。然而，由于AI和机器学习专注于这些人类能完成但难以明确描述的任务，使用比喻来描述AI和机器学习算法变得尤为普遍。为了让计算机检测猫，我们提供了成千上万张猫的图片和非猫的图片作为示例。然后我们运行众多算法中的一种，通过一个具体而详细的数学过程来区分猫和其他物体。但在技术术语中，我们称这个过程为学习。当模型在新图片中未能检测到猫，因为那是一只狮子，而狮子不在最初的猫列表里时，我们常说模型没能理解狮子。确实，每当我们想向朋友解释某事时，常常会利用双方都熟悉的共同概念来打比方。由于AI和机器学习的核心目标在于复制人类执行任务的能力，这些比喻往往使用了暗示人类真实认知功能的语言。随着大语言模型展现出接近人类水平的能力，这些比喻变得弊大于利，因为人们过度解读它们，开始相信它们比实际含义更多。因此，我们将谨慎使用比喻，并提醒读者不要过度联想。有些术语，如学习，是值得理解的技术术语，但我们希望你对它们可能暗示的含义保持警惕。

有时类比在这本书中仍有帮助，但我们会尽量明确解释此类类比的边界。

### 1.5 人类与机器在语言表示上的差异

注：大脑结构的抽象模型已被证明在许多领域都颇为有用。神经网络在语言、视觉、学习和模式识别方面取得了令人瞩目的进展。神经机器学习算法的进步、数字数据的急剧激增以及GPU等计算机硬件的爆发式发展，这三者的融合共同推动了当今ChatGPT的实现。

从这一讨论中得出的关键细节是：作为人类，你对你逐渐习得的语言有着天生的理解。你对语言的学习和使用是交互式的。通过进化，我们似乎都拥有相对一致的相互学习和交流方式。要深入了解这一概念，可以查阅语言学家诺姆·乔姆斯基提出的普遍语法理论。与人类不同，LLM通过静态过程学习语言的表示。当你与Claude或ChatGPT对话时，它机械地参与对话，尽管它从未有过对话经历。LLM习得的语言表示可能质量很高，但并非没有错误。它是可操控的，我们可以通过特定方式改变LLM的行为，以限制其知道或生成的内容。理解LLM通过从示例中推断出的关系来表示语言，有助于我们保持现实的期望。如果你打算使用LLM，它出错时有多危险？你如何利用语言的表示来构建产品或避免不良后果？这些是我们在本书中将要讨论的一些高层次关注点。

### 1.6 生成式预训练变换器及其同类：术语“生成式预训练”

Transformer模型是由OpenAI提出的，用来指代他们在2018年引入的一种新型模型，该模型包含一种称为变换器的神经网络组件。尽管最初的GPT模型（GPT-1）已不再使用，但预训练和变换器的核心思想已成为近期生成式AI及Claude、Gemini、Llama和Copilot等工具革命的核心支柱。同样重要的是，要认识到这些基于GPT的AI工具只是LLM算法研究和应用广阔领域中的一个例子。除了ChatGPT的发布，我们还观察到LLM的惊人激增。一些LLM，如EleutherAI和BigScience研究工坊发布的模型，可免费提供给公众以推动研究和探索应用。如我们之前提到的，Meta、微软和谷歌等公司发布了其他具有更严格许可条款的LLM。任何人都可以用来构建应用或系统的公开可用LLM（有时称为基础模型）已经创造了一个由研究人员、爱好者和公司组成的活跃社区，他们正在探索LLM和生成式AI所带来的应用、局限和机遇。我们在本书中教授的概念几乎适用于所有LLM。每个LLM都使用与ChatGPT相似（如果不是完全相同）的结构来生成输出。一本书要包含适用于许多模型的通用总结似乎是不可能的。然而，这是可能的，原因有几个，其中最重要的是我们不会深入到你从零开始编码LLM所需的深度。自然，ChatGPT和其他商业LLM的部分内容仍然是商业机密。因此，我们的范围和描述有意地概括了当今所有生成式LLM的最常见方面。我们能够给出如此广泛适用总结的第二个原因是LLM的本质。虽然确实可以在构建和运行方式上进行许多调整，但该领域的研究人员一致发现，最重要的细节如下：

模型规模有多大，能否使其更大？构建模型使用了多少数据，能否获得更多数据？

这些结论对于那些自认为拥有重要见解或设计、能够显著改进LLM工作方式的研究人员来说可能令人沮丧，因为在许多情况下，同样的改进只需通过“扩大规模”或使用更多数据、更多参数来构建模型就能轻易实现。扩大模型和数据池的规模是围绕使用和构建LLM的许多伦理问题的关键组成部分，这一点我们将在第9章讨论。

### 1.7 LLM为何表现如此出色

我们将在后续章节详细讨论LLM的工作原理，但在这里也值得分享一个从研究ML算法中学到的关键经验。许多年来，

要想在任意任务上从算法中获得更好的性能，通常需要巧妙设计算法。你会研究问题、数据和数学，尝试推导出关于世界的宝贵真理，并将其编码到算法中。如果做得好，性能就会提升，所需数据减少，一切都很美好。你听说过许多经典深度学习算法，如卷积神经网络（CNN）和长短期记忆网络（LSTM），从宏观来看，它们都是人们深思熟虑、巧妙设计的成果。即使是更简单的“浅层”机器学习算法（如XGBoost），不依赖神经网络或深度学习，也是通过巧妙的算法设计创造出来的。LLM展示了更新的趋势。它们不费心巧妙设计算法，而是保持简单，实现一个仅捕获信息间关系的朴素算法。在许多方面，LLM被迫注入算法的世界观更少。从根本上说，这提供了更大的灵活性。如果我告诉你过去人们正是通过相反的方法来改进算法的，你怎么会认为这样是个好主意呢？区别在于，LLM及类似技术规模更大，大得多。它们在更多数据上训练，并且有更强的能力捕捉更多句子中更多词之间的更多关系；这种蛮力方法似乎在性能上超越了经典机器学习方法。图1.5展示了这一思想。

![图 1.5 如果算法的巧妙程度取决于向设计中编码的信息量，那么旧技术通常通过比前辈更聪明的设计来提升性能。而LLM则选择了更“笨”的方法：使用更多数据和参数，对算法可学习的内容施加最小约束，圆圈的大小反映了这一点。](assets/fig-1-5-d22d00d6f7.png)

*图：图1.5
如果算法的巧妙程度取决于向设计中编码的信息量，那么旧技术通常通过比前辈更聪明的设计来提升性能。而LLM则选择了更“笨”的方法：使用更多数据和参数，对算法可学习的内容施加最小约束，圆圈的大小反映了这一点。*

正如我们已经指出的，大并不代表所有指标都更优。目前，部署这些模型在物流和计算上仍是一大挑战。许多实际约束，包括响应时间、功耗、电池消耗和可维护性，都受到了负面影响。因此，LLM仅在“性能”这一狭义定义上有所提升。

然而，“做大”优于“取巧”这一教训仍然值得我们深思。有时在设计机器学习解决方案时，即使使用了LLM，最佳答案可能仍是“让我们去获取更多数据”。

### LLM实战：善、恶、惧

在本书中，我们将举例说明LLM失败的方式，这些例子往往令人啼笑皆非或显得愚蠢。这些例子的目的并非说LLM无法完成任务。通过改变输入、设置或凭借运气，你往往能让LLM表现更好。这样的例子旨在向你展示LLM可能失败的方式，尤其是在一些连孩子都能做得更好的简单事情上。当你阅读本书并亲自与LLM互动时，这些例子应该会让你停下来思考：“如果我用ChatGPT做困难的任务，但它却在简单的任务上失败，我是不是在自找麻烦？”答案往往是响亮的“是”！安全使用LLM需要对其输出保持一定程度的怀疑或质疑，需要验证正确性，并具备相应调整的能力。如果你用LLM来完成你自己无法完成的任务，你就有可能面临自己无法验证的错误结果。在整本书中，当我们进一步讨论如何使用LLM时，我们将不断穿插这一观点及其应对方法。很容易想象LLM在正常工作时的许多潜在便利——回复你所有的邮件、总结长文档、解释新概念。但很多人没有自然意识到的是，事情可能出错并迅速变得危险。这种对抗性思维通常可以用一个初始例子来引发：比如你想学习如何制造炸弹。如果你问ChatGPT这个问题，你会得到一个规范的回答：“抱歉，我无法协助这个请求。”如果你处于危机或需要帮助，请联系当地政府或专业人士。”然而，研究人员最近展示了如何让ChatGPT和许多其他商业LLM毫不犹豫地回答这个问题，以及其他许多危险的信息请求[4]。有人可能会争辩说，如果有人聪明到能想出让LLM上当的方法，他们很可能也能从其他来源得到他们想要的危险信息。这很可能没错，但与此同时，它没有考虑到LLM和生成式AI工具中的自动化规模。没有哪种AI或ML算法是完美的，如果数百万人提问，LLM可能在0.01%的情况下产生危险回应。ChatGPT拥有超过1亿用户[5]，因此就有1万个危险回应。当你考虑恶意行为者可能开始自动化什么时，问题会更严重。我们将在本书的后半部分进一步讨论这个问题。

我们期待您加入我们，一起探索LLM的工作原理。最终，您将详细理解在您的业务或日常生活中运用LLM革命性能力时需要考虑的诸多事项。

### 小结

ChatGPT 是一种大型语言模型，而大型语言模型本身属于生成式 AI/ML 这一更大的范畴。生成式模型产生新的输出，而 LLM 在输出质量上独一无二，但其制造和使用成本极其高昂。LLM 大致模仿了人类大脑功能和语言学习的不完全理解成果。这仅仅是设计上的灵感借鉴，并不意味着这些模型具有与人类相同的能力或弱点。智能是一个多面向且难以量化的概念，因此很难断言 LLM 是否具有智能。从能力和可靠性角度来思考 LLM 及其潜在用途更为切实。人类语言必须在 LLM 的内部表示之间进行转换。这种表示的形成方式会改变 LLM 学习的内容，并影响你如何基于 LLM 构建解决方案。
...
如第一章所述，在人工智能领域，借鉴人类学习的方式来解释机器如何“学习”往往很有帮助。你阅读和理解句子的过程是一个复杂的认知过程，随年龄增长而变化，并涉及多个顺序和并行认知过程[1]。然而，大型语言模型（LLM）使用的过程比人类认知过程更简单。它们采用基于神经网络的算法，从大量数据中捕捉词语之间的关系，然后利用这些关系信息来理解和生成句子。我们对这些算法工作原理的探讨将从其输入——文本句子——开始。在本章中，我们将探究LLM如何处理这些句子，使其成为模型的输入。正如语言对你思考和加工信息至关重要一样，LLM的输入对于决定其能够执行的概念和任务类型也至关重要。

### 2.1 分词作为数字表示

LLMs 处理句子似乎是理所当然的，但要完全理解，我们必须更加具体。当我们讨论 LLM 的工作方式时，你会看到文本句子对于驱动 LLM 的神经网络算法来说是不自然的，因为神经网络从根本上使用数字来执行其工作。如图 2.1 所示，LLM 采用的算法必须先将人类文本转换为数字表示，然后才能进行处理。分词是 LLM 用来将文本分解成可以编码为数字的片段的一种表示。

![图 2.1 为了理解文本，LLM 必须将文本分解为分词。每个唯一的分词都有一个与之关联的数字标识符。](assets/fig-2-1-1da6a91561.png)

*图：图2.1 为了理解文本，LLM 必须将文本分解为分词。每个唯一的分词都有一个与之关联的数字标识符。*

你可以把 token 想象成大语言模型处理文本的最小单位——一个“原子”，如果愿意这么类比的话，它是构成其他一切的最小组件。那么，文本的原子是什么？想一想：当你阅读这本书时，你的大脑用来处理意义的最小构建块是什么？两个自然的答案是字母和单词。我们很容易把字母定义为原子，因为单词由字母组成，但是你真的会有意识地阅读每个单词中的每个字母吗？对大多数人来说，答案是“不”。（如果你和本书的合著者之一一样有阅读障碍，这个问题会很奇怪。但认知处理复杂且未被完全理解，请容忍我们的类比！）你关注的是更显著的单词和单词部分。实际上，你很可能能理解这句话，即使我们没用正确的拼写或字母。人们无意识地使用单词的部分来处理文本，大语言模型也基于同样的原理构建。在本章中，你将学习将文本转换为 token 的过程。首先，我们将更详细地讨论 token；然后，讨论决定句子如何转换为 token 的程序。

### 2.2 语言模型只看到分词

大多数说英语的成年人到成年时大约认识30,000个单词[2]。最初为ChatGPT提供动力的LLM——GPT-3，拥有50,257个token的词汇量[3]。这些token并非单词，而是单词的组成部分，称为子词（subwords），这是一种介于单词和字母之间的表示。直观上，一个token捕捉了语言的最小有意义语义单元。例如，单词 `schoolhouse` 通常会被拆分为 `school` 和 `house` 两个token，而 `thoughtful` 则拆分为 `thought` 和 `ful`。这有助于识别常见词汇，并利用子词来解读从未见过的新词。人们经常使用一种类似的技术，称为语义分解（semantic decomposition），来理解从未见过的单词。我们直观地将新词拆分为组成部分，基于已经理解的词汇来把握其含义。特征工程是将数据转换为更适合算法和任务形式的过程。要构建一个能检测给定文本语言的算法，你可以编写代码，将文本作为输入，输出每个字符出现的百分比。例如，如果文档中频繁出现 `é`，那么你就有了一个很好的特征，表明该文档更有可能是西班牙语或法语，而不是俄语或中文。优秀的特征工程需要思考模型如何工作、你想要实现什么，以及如何为模型与目标的结合准备数据。分词（tokenization）就是LLM的特征工程；它至关重要，因为token是模型唯一与之交互的信息。token被视为独立的、抽象的事物，本质上是没有关联的。它们之间的关系是通过观察数据学习得到的。回顾图2.1，很明显，`Dis` 和 `dis` 的token是相关的，唯一的区别在于一个以大写D开头。然而，你可以看到，模型将标识符4944分配给 `Dis`，将834分配给 `dis`。也就是说，模型本身看不到表示 `Dis` 和 `dis` 的token之间的任何联系，即使我们人类能看到明显的联系。模型甚至看不到 `Dis` 或 `dis`。为了让LLM处理token，我们必须将这些token转换为数字，这样模型就会看到数字4944和834。重要的是，模型没有任何直接方式知道这些token是相关的。token是从子词到唯一数值表示的映射。相应地，分词是将完整文本字符串转换为token序列的过程。如果你之前使用过机器学习库（尤其是任何自然语言处理[NLP]工具），你可能对某些更简单的分词形式有所了解。例如，一个简单的分词过程通过基于空格拆分文本来将文本分割成token。然而，这种方法限制了创建子词的能力，也难以处理不使用空格分隔单词的语言，比如中文。

### 2.2.1 分词过程

分词遵循的一般过程如图2.2所示，包含四个关键步骤：

1. 接收待处理文本——即从用户、互联网或任何包含所需文本的源获取字符串类型（字母、数字或符号的集合）的文本输入。
2. 转换字符串——这通常涉及以某种有用的方式更改字符串，例如将大写字母转换为小写。这样做也可能是出于安全原因（例如，文本来自用户，需要移除任何可能看起来像恶意输入的内容），或者是为了消除文本中不相关的变异，以帮助算法更好地学习。这一过程被称为归一化。
3. 将字符串拆分为词元——一旦获得字符串，就需要将其拆分为一系列离散的子字符串；这些就是在更大字符串中找到的词元。这被称为分词。
4. 将每个令牌映射到唯一标识符——唯一标识符通常是一个整数，生成LLM可理解的输出。

![图 2.2 通常，令牌化涉及处理输入以生成令牌的数字标识符。](assets/fig-2-2-1353cf4e5f.png)

*图：图2.2 通常，令牌化涉及处理输入以生成令牌的数字标识符。*

这一过程的首尾两端几乎没有选择余地或不同行为的空间。首先，你需要输入进行加工；最后，你需要为每个词元分配一个数字标识符，以便存储和检索与这个词元关联的信息。中间的两个步骤——归一化和分词——才是你可以自主选择处理方式的地方。

分词过程的最后一步是构建词汇表。模型的词汇表是指在训练过程中，当我们将数据交给算法学习时，所看到的唯一令牌的总数。要构建一个包含大量唯一令牌的丰富词汇表，几乎总是需要大量数据。为模型选择词汇表涉及一系列权衡：词汇表越大，模型能成功处理的信息就越多。比如一个只有几十个单词词汇量的一岁小孩。这个孩子并不是一个非常有效的沟通者（不过这没关系；他们还有很多时间去学习）。因此，更大的词汇量不仅有助于模型理解更多事物，也会使模型变得更大。如果词汇表过大，可能会因为使用它所需的计算量而使模型变慢，或者模型可能消耗过多的内存或磁盘存储空间，导致难以传输或共享到其他机器——例如，当作为软件应用的一部分部署时。通过处理训练数据并识别令牌来构建模型的词汇表。每遇到一个新令牌，就根据已看到的唯一令牌数量给它分配一个唯一标识符。这个过程通常很简单：存储一个初始为0的计数器，每次发现新令牌时递增它。一旦过程完成，你就拥有了一个实际上充当编码器的分词器。分词器可以接收文本作为输入，并返回该文本的数值编码，供LLM算法作为输出使用。

### 2.2.2 控制分词中的词汇表大小

GPT-NeoX 是一个公开可用的大语言模型，其词表存储在磁盘上约需 10 GB。数据量非常大，从数据存储和计算的角度来看，已经足以让许多实际应用场景变得困难。词表如此之大，以至于存储在 micro-SD 卡上会慢得不可接受，从而在手机或某些游戏机上的使用成为重大挑战。而且大到无法实时流式传输，必须下载并加载到处理器 RAM 中才能执行分词。然而，词表必须足够大，以表示模型在训练和使用过程中可能遇到的所有单词和子词。假设模型遇到一个不在其词表中的单词，且无法通过词表中的子词组合来表示。在这种情况下，模型无法获取该文本片段的信息。因此，必须权衡词表大小与模型解释多样内容的需求。在自然语言处理中，这通常称为“词表外词问题”，即遇到模型可用 token 无法表示的单词。词表大小是影响大语言模型大小的因素之一，因此讨论控制词表大小的方法和权衡至关重要。在本节中，我们将描述改变分词过程的行为如何影响词表大小，进而影响模型的能力和准确性。

![图 2.3 标准化过程通常涉及将文本中的大写字符和标点符号移除。](assets/fig-2-3-4971a990f1.png)

*图：图 2.3 标准化过程通常涉及将文本中的大写字符和标点符号移除。*

在图2.3中，我们聚焦第二个转换步骤——归一化，该步骤将大写字符“H”和“W”转换为小写并移除标点符号。这些常见的归一化步骤源自经典的NLP管线，如今在现代深度学习方法中有时仍会使用。它们立即可喜的效果是减小词汇量。无需将“Hello”和“hello”表示为两个独立的词元，而会被映射到同一个唯一的词元。这种映射意义重大，因为每个句子开头的大写单词原本可能会在词汇表中产生一个大写版本的重复词。这种归一化还有助于处理各种拼写错误和笔误。例如，在撰写本书时，我们曾输入“LLMs”、“LLms”和“llms”等各种大小写混合的笔误。将每种变体中的每个字符转换为小写，就能将所有笔误归并为一种简单形式，从而获得更小的词汇量并降低歧义。然而，将文本转换为小写并不总是能降低歧义。考虑“Bill”和“bill”的情况。在前一种情况中，大小写至关重要，它表明“Bill”很可能是一个人名，而“bill”更可能指代货币单位（或“bill”的其他定义之一）。大小写不仅对理解文本的含义至关重要，也对识别文本中的错误至关重要。再次考虑我们在本书中错误使用“LLMs”大小写的各种方式。一个高质量的AI算法应该能够识别出我们犯了笔误并进行纠正！ChatGPT能够做到这一点，因此模型中需要保留大小写信息。因此，在词汇量和潜在模型准确性之间存在一个重要的权衡需要考虑。在经典的NLP乃至BERT（驱动ChatGPT的LLM的前身）等不算太旧的深度学习模型中，除了专门针对该目的设计的方案外，算法识别并修正笔误的能力极为有限。因此，过去用于设计稳健归一化步骤的大部分工作如今已被LLM舍弃。更庞大的词汇量是可取的，以便训练出能够学习理解错误的更强大的模型。

### 2.2.3 词元化详解

标记化过程中的规范化与分词步骤在很大程度上决定了词汇量的大小。图2.4展示了一种最直接的标记化策略。该策略遵循一个简单规则：每当文本中出现空格时，就将较大的字符串拆分为那些标记。以"hello world"为例，只需在Python中调用"hello world".split(" ")即可。这是一种合理的方式，也是我们人类阅读句子的方式。但它也引入了一些微妙的复杂性。

![图 2.4 分词过程将规范化文本拆分为单词或标记，以便每个单词或标记都能被独立处理。](assets/fig-2-4-f3ba3f9e0b.png)

*图：图2.4
分词过程将规范化文本拆分为单词或标记，以便每个单词或标记都能被独立处理。*

文本中出现标点符号会怎样？如果我们使用空格规则将字符串“hello, world”切分为["hello,", "world"]，就会遇到与大小写相似的问题。最终同一个概念"hello"与"hello,"会被表示为两个不同的token。传统方法通常通过删除标点或制定更复杂的分词规则来解决这个问题。虽然这朝着缩减词表大小迈出了正确一步，但手动指定分词规则并不能解决其他问题。例如，对于中文这种不使用空格分隔词语的语言，基于规则的分词策略会带来巨大挑战。

### 使用字节对编码识别子词

LLM的总体思路是减少手工特征工程，让算法承担繁重工作。因此，通常使用字节对编码（BPE）算法将字符串拆分为token。字节对编码是一种将单词拆分为常见子词序列的算法。当今的BPE通常使用自定义分词器，几乎不进行归一化。

注意：通过实验我们发现，许多类似ChatGPT的产品会移除一些不可打印的Unicode字符（Unicode确实奇怪），但除此之外基本保持你的文本原样。而大多数之前的语言模型确实使用了各种形式的规范化，如何更好地为LLM规范化文本，我们认为是一个值得探讨的开放性问题。

由于找到最高效的子词集在计算上很昂贵，BPE 采用一种启发式方法来走捷径。它首先将单个字母视为token，然后找出出现最频繁的相邻字母对，并将它们组合成子词token。该算法重复多次，继续处理子词token，直到达到某个阈值，词汇表变得“足够小”。例如，在第一轮中，BPE 算法检查英语中单个字母的频率，并发现字母“i”、“n”和“g”经常相邻出现。在第一轮中，BPE 可能会观察到“n”和“g”一起出现的频率高于“i”和“n”，因此它会生成 token i 和 ng。在后续轮次中，它可能会根据该字母组合与“ng”与其他字母或子词出现的频率对比，将这些 token 组合成 ing。一旦 BPE 达到停止点，它将识别出诸如“eating”和“drinking”等经常出现的组合作为完整单词。它还可能将“ing”作为一个后缀捕获，从而使以此子词结尾的其他单词也能表示为 token。算法完成后，我们得到的 token 既有表示完整单词的，也有表示子词的。图2.5 高层级地展示了这一过程。

![图 2.5 一个简化的字节对编码算法用于创建token：首先，找到最常出现的字符对“ng”。然后，将所有“ng”替换为占位token “T”，并将“ng”加入词汇表。重复此过程，直到没有常见的字节对为止。](assets/fig-2-5-b804ac34a1.png)

*图：图2.5 一个简化的字节对编码算法用于创建token：首先，找到最常出现的字符对“ng”。然后，将所有“ng”替换为占位token “T”，并将“ng”加入词汇表。重复此过程，直到没有常见的字节对为止。*

注意：运行BPE算法创建词表出奇地昂贵，因为它必须多次读取输入数据来计算最常见的字母组合。虽然LLM在超过5亿甚至10亿页文本上训练，但它们的分词器通常只使用其中一小部分数据来创建。通常，分词器是在规模小得多（如一本小说大小）的文本集合上训练的。

BPE 过程初看可能有些怪异，但你可以将其视为识别语料库中常见字符串的一种方法。例如，BPE 几乎总能学会将 "New York" 表示为一个 token，这很有用，因为纽约州和纽约市在文本中频繁出现。将整个概念表示为一个 token 使得利用这类信息更加容易。实际上，大多数常见词会成为唯一的 token，而稀有词则有望通过子词的组合来捕获。例如，"loquacious" 会被 GPT-4 分词为 lo、qu 和 acious。这种方法之所以成功，是因为 "acious" 是表示倾向/习性的拉丁语后缀，这使得模型更容易正确处理一个非常见词。它也是一个失败案例，因为拉丁语前缀 "loqu" 被拆成了两个 token 而非一个，增加了学习难度。使用 BPE 构建词表后，模型作者会出于各种原因手动添加额外的 token，例如对特定知识领域重要的词汇。正如我们将在下一节讨论的，在某些领域，拥有正确的 token 通过捕获细微含义可以产生显著效果。因此，作者通常会确保包含必要的 token。模型作者还会添加一些特殊 token，它们并不直接表示单词部分，而是向模型提供辅助信息。一些常见的例子是 "unknown" token（通常表示为 [UNK]），当分词器无法正确处理某个符号时使用；以及系统 token [SYSTM]，用于区分模型的内置提示和用户输入的数据，以及其他类型的风格标记。接受文本和图像输入的多模态模型使用独特的 token 来告知模型输入流何时在表示文本数据的字节和表示图像数据的字节之间切换。OpenAI 在开发 ChatGPT 时决定使用 BPE 将文本编码为 token，并已将分词器作为开源包 tiktoken 发布（https://github.com/openai/tiktoken）。尽管如此，还有其他几种用于自动生成 token 的算法和实现可用，包括 Google 开发的 WordPiece 和 SentencePiece 算法 [4]。每种都有不同的权衡。例如，WordPiece 在构建分词词表时使用了不同的技术来统计候选子词的出现频率。SentencePiece 中实现的算法之一是处理整个句子，在计算 token 时保留空格，这可能会提升处理多种语言模型的输出效果。然而，BPE 是使用最广泛的算法。例如，它目前被 Google 最新的 LLM 独家使用。无论选择哪种算法，分词器的词表大小都是一个关键模型参数，由负责训练和增强分词器的数据科学家或工程师决定。接下来的章节将深入探讨有关词表大小以及在分词器开发过程中所做的其他决策中的一些考量。

### 2.2.4 分词的风险 如第一章所述，本书不会过多涉及编码。我们的目标是让你对LLM的工作原理有合理的理解，并去除一些神秘感，从而让你能专注于LLM如何应用于你的工作。分词是解开这个谜题的第一块拼图。它是一种简单但有效的策略，用于生成LLM的输入。你已经了解到，词汇量大小对模型的可部署性起着重要作用，在识别细微差别与构建词汇表带来的不必要冗余之间存在权衡，分词过程如何影响词汇量大小，以及如何通过BPE自动选择分词。分词时所做的选择会影响LLM当前和未来的能力。这些选择涉及一些宏观层面的挑战，值得注意。为了进一步探讨这一话题，BPE有两个显著但又微妙的细节值得关注：句子长度与分词数量之间的关系，以及LLM可能被那些看似相同但二进制编码不同的字符（即同形异码字符）混淆的潜在风险。

![图 2.6 对两个不同句子的分词](assets/fig-2-6-a899601062.png)

*图：图2.6
对两个不同句子的分词*

这种差异的产生是因为BPE贪婪地为任何输入片段寻找最小的token集。在这个具体案例中，字符串“running”在训练数据中出现得足够频繁，因此它拥有了自己的token。当缺少“g”时，我们的词汇表中没有“runnin”的token，因为这种变体在训练数据中可能很少出现。因此，“runnin”需要被拆分成至少两个token，得到run和nin。分词器实现的这一细微之处是滋生软件bug的温床。不同的分词器在对同一字符串进行分词时可能给出不同的结果。在设计单元测试和基础设施时，务必牢记这一因素，以避免在升级或切换分词器实现时迷失或困惑，因为这些操作可能导致token生成出现新的差异。它还会影响LLM的评估，因为许多模型对添加的空格高度敏感，不一致的分词可能无意中导致比较不公平。

### 同形字造成混淆

同形异码字符（homoglyph）是开发者在处理多种人类语言或考虑处理外部数据的安全隐患时可能遇到的问题。当输入来自任意用户时，有时可能带有恶意，试图欺骗你的模型做出不良行为。针对LLM的一种攻击方式就是同形异码攻击。同形异码是指两个或多个字符具有不同的字节编码，但在屏幕上渲染时看起来完全相同。例如，大多数西欧语言中使用的拉丁字母“H”与东欧和中亚广泛使用的西里尔字母“H”就是一组同形异码。BPE会将采用不同字节编码的同形异码编码为不同的token。因此，同形异码会增加文本中的token数量，改变LLM解析信息的方式，并推高你的计算成本。一个有趣的同形异码例子是Unicode字符U+200B，即“零宽空格”。该字符用于排版，占据空间，但不会打印任何内容、显示任何东西，也不会改变文档的渲染方式。零宽空格是Unicode规范中众多奇特有趣的东西之一，可能会给你带来麻烦。因此，许多服务会采用归一化步骤，移除这类奇怪字符，并将同形异码替换为规范表示（例如，任何看起来像“a”的字符都必须编码为a）。例如，OpenAI当前的tokenizer接口会移除同形异码。如果你打算在自己的硬件或用户设备上部署LLM，就必须考虑同形异码问题。

### ## 2.3 分词与LLM能力

如果我们只关心LLM生成高质量人类文本的能力，那么文本分词的具体细节就不如构建这些模型所用的数据和算力重要。如果你为模型投入足够的计算能力和规模，它们最终会学到有用的表示，无论基础构建块是什么。但有时，分词会显著影响LLM的能力。本节将介绍一些例子。接下来的例子可能与你当前的工作或LLM的应用场景并不直接相关。这完全没问题；这些例子并不是要劝退你使用LLM。相反，目的是帮助你理解LLM学习到的范围受限于所选的表示方式，而且如果不进行重大的工程改造，这些问题可能无法绕过。如果你开始用LLM构建应用并遇到重大困难，思考一下分词如何成为实现目标的阻碍。如果确实是分词导致的问题，你几乎无法自行解决，因此最好考虑其他方法，比如手动扩充词汇表，加入对你的应用重要的 token。

### 2.3.1 LLM不擅长文字游戏

用户经常喜欢让LLM解字谜或完成涉及文字游戏的任务。例如，图2.7展示了一个文字游戏，其正确答案取决于单词的精确字母序列和字母数量。

![图 2.7 分词方法意味着ChatGPT实际上无法“看到”单个字符或单词长度。如果你提出需要识别子字符的问题，并以独特和不寻常的方式改变它们，ChatGPT就会开始出错。正确的中间字符是“a”，但ChatGPT坚持认为是“e”。ChatGPT看到的是三](assets/fig-2-7-7b754dd023.jpg)

*图：图2.7 分词方法意味着ChatGPT实际上无法“看到”单个字符或单词长度。如果你提出需要识别子字符的问题，并以独特和不寻常的方式改变它们，ChatGPT就会开始出错。正确的中间字符是“a”，但ChatGPT坚持认为是“e”。ChatGPT看到的是三个token，分别代表P、ine和apple。*

玩文字游戏可能不是你在应用中关心的事，但文字游戏失败的原因可能对你的问题高度相关。尽管这类例子大多是玩具问题，不具有特别的科学或商业重要性，但它们揭示了这些模型运行中的显著故障。它们可能在实际使用中出现，比如当模型难以创作包含押韵或谐音的诗歌时。例如，考虑你正在构建一个回答用户处方药问题的应用。药物名称通常又长又容易混淆，用户记不住或拼写错误，而由于LLM不理解字母，它可能会将一种药物的名称误认为是另一种药物又长又奇怪的名字。因为药物名称不常见，即使有轻微拼写错误，它们的分词方式也会不同。例如，在GPT-3中，“Amoxicillin”和常见的拼写错误“Amoxicillan”没有共同的token！这导致LLM错误回答的风险更大，而风险本身更高，使得LLM应用更需要彻底测试、极端谨慎地工程化绕过，或者干脆完全避免。

### 2.3.2 LLM在数学方面面临挑战

分词显著影响涉及形式符号推理的任务，包括数学和下棋。数学和下棋都是 LLM 作为符号推理问题实现的，其中单个令牌在与其他令牌共同出现时，其互动和含义受特定规则支配。例如，为每个数字包含单个令牌的模型在算术上往往表现优于不含单个数字令牌的模型。这是因为数字 123456 在 GPT-3 中会根据分词器原始训练数据中这些令牌的频率变成两个令牌：["123", "456"]。这使得模型更难处理该数字中的各个数位。一些系统开发者通过在所有数字之间插入空格来规范化数字，例如 1 2 3 4 5 6，从而生成包含六个令牌的新输出（每个数字一个令牌），解决了这个问题。这种数学能力的差异在图 2.8 中得到了充分说明，该图显示了整个训练过程中算术计算的表现。上方的曲线是典型的 BPE 分词器，而下方表现更好的曲线是同一分词器经过修改、实现数字级别分词的结果。

![图 2.8 比较了两个LLM随时间学习算术计算的表现。x轴表示时间。上方曲线是典型的BPE分词器，下方曲线是同一分词器经过修改、使用代表单个数字的令牌。y轴描述LLM准确执行的能力，数值越小表示错误越少。结论是：使用数字级分词的LLM能更好更快地学习数](assets/fig-2-8-0a958765b5.jpg)

*图：图2.8 比较了两个LLM随时间学习算术计算的表现。x轴表示时间。上方曲线是典型的BPE分词器，下方曲线是同一分词器经过修改、使用代表单个数字的令牌。y轴描述LLM准确执行的能力，数值越小表示错误越少。结论是：使用数字级分词的LLM能更好更快地学习数学。*

### 2.3.3 LLM与语言公平

大多数LLM分词器能够表示Unicode覆盖的任何符号，这包括世界上大多数语言的字符。然而，这些分词器在特定语言中表示文本的效率差异巨大，尤其是在分词器通常针对不同语言的较小文本资源集进行训练的情况下。这可能导致基于LLM的商业服务出现显著不公平[5]，因为训练集中稀有语言的单词分词默认会采用更细粒度的子词集，从而增加令牌使用量。像OpenAI和Anthropic这样的商业LLM提供商通常向客户收费

按每个令牌计费，通常每个输入或输出的令牌只需几分之一美分。这些成本累积起来，考虑到一个高使用量的商业应用每天可能处理数千万个令牌。LLM完成一个请求所需的时间以及用户按令牌支付的费用，直接取决于分词器。因此，那些能被分词器更高效表示的语言，在经济上比表示效率低下的语言更具优势。以英语为基准，研究人员发现，使用ChatGPT和GPT-4时，回答德语或意大利语用户查询的成本高出约50%。与英语差异更大的语言可能产生更高的费用：Tumbuka语和保加利亚语的成本是英语的两倍以上，而宗卡语、奥里亚语、桑塔利语和掸语的处理成本是英语的12倍以上。

### 2.4 检查你的理解

1 你期望以下单词或短语如何被分词？尝试自己拆分它们，然后通过一个实际的大语言模型分词器（例如 https://platform.openai.com/tokenizer）运行：

backstopped large language models Schoolhouse 你处理句子以理解它们的过程是一个复杂的过程，会随着年龄增长而变化，涉及多个顺序和并行的认知过程。2 你认为对于之前的每个示例，大写和小写字母有多大影响？尝试用各种大小写形式再次提交它们。

既然 token 是 LLM 操作的基本单元，那么从技术上讲，为什么一种语言被分词器表示的效率越低，其成本反而更高呢？

LLM 根据用户所说的语言对同一服务收取不同费用，这是否是一个伦理问题？你会认为这是一种歧视吗？

### 2.5 上下文中的分词

我们在这章讨论的分词细节是LLM的基础构建模块，它们决定了LLM能够有效表示的输入和生成的输出。分词是像ChatGPT这样的LLM的关键组成部分，用于开发文本的有效表示，以便在训练过程中面对海量信息时能够学习token之间的关系，解读用户输入并生成我们习以为常的高质量响应。LLM的潜力受限于或得益于其采用的分词策略和词汇表，以及我们在后续章节中探讨的其他所有特征。

### 总结

分词是将句子转换为标记的过程，是大语言模型理解文本的基础。标记是文本中代表内容的最小信息单元。有时它们对应完整的单词，但更多时候表示单词片段或子词。分词涉及将文本规范化为标准表示形式，例如将字符转换为小写，或转换Unicode字符的字节编码，使外观相同的字符采用同一编码。分词还包含分割步骤，即将文本拆分为单词或子词。字节对编码（BPE）等算法提供了一种机制，可根据训练数据集中字母组合的统计出现频率，自动学习如何高效分割文本。构建分词器的结果称为词汇表，它是分词器用来表示所处理文本的所有单词和子词标记的唯一集合。分词器词汇表的大小影响大语言模型准确表示数据的能力，以及理解和预测文本所需的存储和计算资源。在大语言模型内部，标记用数字表示。因此，模型无法理解标记之间的关系，例如前缀和后缀，或两个标记共享相似字母集的事实。为支持特定知识领域，自动训练的分词器可以扩充，以提供对其应用重要的标记。不理解单个字母或数字的分词器在处理算术运算或简单文字游戏时会出现问题。
...
![图 4.1 投资回报难以预测，部分原因在于它们并非平滑。（图像修改自[2]，遵循知识共享许可协议。）](assets/fig-4-1-c011ee570d.jpg)
...
![图 4.2 左侧是一个平滑函数的示例，右侧是两个非平滑函数的示例。中间的示例大部分是平滑的，但有一个区域不光滑，因为函数在该处无值。右侧的函数由于值的剧烈变化，处处非平滑。](assets/fig-4-2-cc90c84d81.png)

*图：图4.2 左侧是一个平滑函数的示例，右侧是两个非平滑函数的示例。中间的示例大部分是平滑的，但有一个区域不光滑，因为函数在该处无值。右侧的函数由于值的剧烈变化，处处非平滑。*
...
![图 4.3 使用输入和标签（每个输入已知的正确答案）来在梯度下降过程中调整神经网络。网络由参数构成，每次应用梯度下降时，这些参数都会发生少量改变。通过应用数百万或数十亿次梯度下降，我们最终将网络转变为有用的东西。](assets/fig-4-3-26f55ee042.png)

*图：图 4.3 使用输入和标签（每个输入已知的正确答案）来在梯度下降过程中调整神经网络。网络由参数构成，每次应用梯度下降时，这些参数都会发生少量改变。通过应用数百万或数十亿次梯度下降，我们最终将网络转变为有用的东西。*
...
![图 4.4 此为梯度下降应用于单参数问题的全局宏观图景。曲线展示了给定参数值下的损失函数值。球的位置显示了当前参数值对应的损失。目标是找到对应全局最小值的参数值，即损失最小的理想解。](assets/fig-4-4-ab19356453.png)
...
![图 4.5 图4.5** 梯度下降算法通过逐步调整参数，寻找损失最小的最优结果。不幸的是，算法会陷入局部最小值，即图中并非最优的区域，因为其他参数值对应着损失更低的区域。](assets/fig-4-5-be9aa69f44.png)

*图：图4.5 **图4.5** 梯度下降算法通过逐步调整参数，寻找损失最小的最优结果。不幸的是，算法会陷入局部最小值，即图中并非最优的区域，因为其他参数值对应着损失更低的区域。*
...
![图 4.6 LLM 会看到这个句子九次，每次都从九段序列末尾的单个词预测中学习。](assets/fig-4-6-9238774812.jpg)
...
![图 4.7 上下文有助于对下一个词做出合理的预测。从左往右看，句子中可能出现的额外文本逐步加入。每个句子气泡中的图片展示了新增上下文如何排除预测。](assets/fig-4-7-5608d47f5b.png)

*图：图4.7 上下文有助于对下一个词做出合理的预测。从左往右看，句子中可能出现的额外文本逐步加入。每个句子气泡中的图片展示了新增上下文如何排除预测。*
...
![图 4.8 虽然预测下一个词很强大，但它并未赋予网络推理或逻辑能力。如果我们向ChatGPT提出荒谬不实的问题，它会兴致勃勃地解释这是怎么回事。](assets/fig-4-8-ee055af12f.jpg)

*图：图4.8 虽然预测下一个词很强大，但它并未赋予网络推理或逻辑能力。如果我们向ChatGPT提出荒谬不实的问题，它会兴致勃勃地解释这是怎么回事。*
...
![图 4.9 由于LLM的训练方式，ChatGPT未能解决经典逻辑谜题的两个修改版本。经常以相同常见形式出现的内容（例如一个著名逻辑谜题）会导致模型照搬常见答案。即使内容发生了对人而言显而易见的重要修改，这种情况也可能发生。](assets/fig-4-9-f9f8fb05c6.jpg)

*图：图4.9 由于LLM的训练方式，ChatGPT未能解决经典逻辑谜题的两个修改版本。经常以相同常见形式出现的内容（例如一个著名逻辑谜题）会导致模型照搬常见答案。即使内容发生了对人而言显而易见的重要修改，这种情况也可能发生。*
...
![图 4.10 对话代理在游戏开始时并不确定具体对象。](assets/fig-4-10-d5bd35a780.png)

*图：图4.10 对话代理在游戏开始时并不确定具体对象。*
...
![图 5.1 可以在四个干预点来改变或约束LLM的行为。图中间展示了模型训练的两个阶段，模型参数在此阶段被修改。左侧，也可以在模型训练之前修改训练数据。右侧，可以在模型训练之后拦截模型输出，并编写代码来处理特定情况。](assets/fig-5-1-89784e7b2d.png)
...
![图 5.2 像 ChatGPT 这样的商用大语言模型被设计成遵循指令（在一定限度内），并能以非常高的效率执行大量低认知或模式匹配任务。这些任务包括模式匹配等程式化写作，以及扮演汽车销售员等指令遵循任务。](assets/fig-5-2-e59bee1407.jpg)

*图：图5.2 像 ChatGPT 这样的商用大语言模型被设计成遵循指令（在一定限度内），并能以非常高的效率执行大量低认知或模式匹配任务。这些任务包括模式匹配等程式化写作，以及扮演汽车销售员等指令遵循任务。*
...
![图 5.3 监督微调（SFT）是改善模型结果的简单方法。你重复构建基础模型的相同过程：基础模型在大量通用数据上训练完成后，再继续在较小的专业数据集上训练。](assets/fig-5-3-001f3716b9.png)

*图：图5.3 监督微调（SFT）是改善模型结果的简单方法。你重复构建基础模型的相同过程：基础模型在大量通用数据上训练完成后，再继续在较小的专业数据集上训练。*
...
![图 5.4 强化学习涉及迭代交互，行动奖励可能很久都不会显现，需要多个步骤才能获得。对于像ChatGPT这样的聊天机器人，环境是与用户的对话，行动是ChatGPT可能生成的无限文本。在某种意义上，奖励变成了用户对聊天机器人对话结束时的满意度。](assets/fig-5-4-5c2e0644f2.png)

*图：图5.4 强化学习涉及迭代交互，行动奖励可能很久都不会显现，需要多个步骤才能获得。对于像ChatGPT这样的聊天机器人，环境是与用户的对话，行动是ChatGPT可能生成的无限文本。在某种意义上，奖励变成了用户对聊天机器人对话结束时的满意度。*
...
![图 5.5 RLHF非常擅长让LLM避免已知的特定问题。然而，它并不能为模型提供处理新问题的工具。在询问迈阿密足球后，谈论迈阿密海豚作为合乎逻辑的下一句话，这种倾向违反了第一个要求——永远不要提及海豚。](assets/fig-5-5-781cb8a647.jpg)

*图：图5.5 RLHF非常擅长让LLM避免已知的特定问题。然而，它并不能为模型提供处理新问题的工具。在询问迈阿密足球后，谈论迈阿密海豚作为合乎逻辑的下一句话，这种倾向违反了第一个要求——永远不要提及海豚。*
...
![图 5.6 RLHF的一个天真且不完整的版本。虚线表示从一个组件发送到另一个组件的文本。由于文本与梯度下降不兼容，因此必须使用更困难的RL算法。这使我们能够根据LLM输出的质量评分来调整LLM的权重。](assets/fig-5-6-9d81a060d3.png)
...
![图 5.7 图 5.7](assets/fig-5-7-47a03b8235.png)
...
![图 5.8 RLHF的完整版本。虚线表示文本，需要通过RL来更新参数。原始LLM是未经任何修改的基础模型，而待微调的LLM从基础模型开始，但经过修改以提高其输出质量。相似度奖励和质量奖励组件均接收词概率以改进计算。RL通过组合质量得分和相似度得分来调整](assets/fig-5-8-79eacad7df.png)

*图：图5.8 RLHF的完整版本。虚线表示文本，需要通过RL来更新参数。原始LLM是未经任何修改的基础模型，而待微调的LLM从基础模型开始，但经过修改以提高其输出质量。相似度奖励和质量奖励组件均接收词概率以改进计算。RL通过组合质量得分和相似度得分来调整参数。*
...
![图 5.9 除了微调，你还可以通过修改训练数据、修改基础模型训练过程，或编写代码处理特定情境来改变模型输出，从而改变模型行为。](assets/fig-5-9-44207cd52f.png)

*图：图5.9 除了微调，你还可以通过修改训练数据、修改基础模型训练过程，或编写代码处理特定情境来改变模型输出，从而改变模型行为。*
...
![图 5.10 图 5.10** 通过编写强制执行格式规范的代码，可以在 LLM 生成输出时捕获无效输出。一旦检测到，让 LLM 生成下一个最可能的 token 直到找到有效输出，是一种简单的改进方法。](assets/fig-5-10-6bb05dd5c7.png)

*图：图5.10 **图 5.10** 通过编写强制执行格式规范的代码，可以在 LLM 生成输出时捕获无效输出。一旦检测到，让 LLM 生成下一个最可能的 token 直到找到有效输出，是一种简单的改进方法。*
...
![图 5.11 左侧展示了用户询问如何编写JSON时LLM的正常使用方式。 LLM自然有可能产生错误输出，这是我们希望最小化的。右侧展示了RAG方法。通过使用搜索引擎，我们可以找到与查询相关的文档，并将它们组合成一个新的提示，从而为LLM提供更多信息和上下](assets/fig-5-11-27edf530b9.png)
...
![图 6.1 图 6.1](assets/fig-6-1-8403d45453.png)
...
![图 6.2 展示了用编程语言Python（左）和Java（右）编写的两个相似代码样本。这些样本展示了字节对编码如何跨不同语言识别相似的词元。方框代表单个词元。人类语言的标准分词方法对代码也能取得不错的效果，因为代码与自然语言有许多相似之处。](assets/fig-6-2-448d4535b8.png)

*图：图6.2展示了用编程语言Python（左）和Java（右）编写的两个相似代码样本。这些样本展示了字节对编码如何跨不同语言识别相似的词元。方框代表单个词元。人类语言的标准分词方法对代码也能取得不错的效果，因为代码与自然语言有许多相似之处。*
...
![图 6.3 用Java编写的代码，其中包含一条描述代码功能的注释。因为（好的）代码（但愿如此）含有大量注释，所以自然语言和代码会自然而然地混合在一起，供LLM利用来获取信息。当变量使用描述性名称时，模型就更容易将代码信息与注释和变量名所描述的意图关联起](assets/fig-6-3-862aec302c.png)

*图：图6.3 用Java编写的代码，其中包含一条描述代码功能的注释。因为（好的）代码（但愿如此）含有大量注释，所以自然语言和代码会自然而然地混合在一起，供LLM利用来获取信息。当变量使用描述性名称时，模型就更容易将代码信息与注释和变量名所描述的意图关联起来。*
...
![图 6.4 开发用于代码的 LLM 需要应用多轮微调。标准训练程序（如第 4 章所述）产生初始的基础 LLM。使用大量代码进行 SFT 可以创建一个擅长处理代码的 LLM。将 RLHF 作为第二个微调步骤可以提升 LLM 生成代码的能力。](assets/fig-6-4-28bc964067.png)

*图：图 6.4 开发用于代码的 LLM 需要应用多轮微调。标准训练程序（如第 4 章所述）产生初始的基础 LLM。使用大量代码进行 SFT 可以创建一个擅长处理代码的 LLM。将 RLHF 作为第二个微调步骤可以提升 LLM 生成代码的能力。*

![图 6.5 一个Python代码示例，其中当前已生成令牌if(A > B)。如果LLM生成的下一个令牌是换行符，则将发生语法错误，因为if语句必须以冒号结尾才有效。对每个新令牌进行语法检查可以捕获此错误，并强制LLM选择一个不会导致语法错误的替代令牌。](assets/fig-6-5-c84e629e44.png)

*图：图6.5 一个Python代码示例，其中当前已生成令牌if(A > B)。如果LLM生成的下一个令牌是换行符，则将发生语法错误，因为if语句必须以冒号结尾才有效。对每个新令牌进行语法检查可以捕获此错误，并强制LLM选择一个不会导致语法错误的替代令牌。*
...
![图 6.7 Minerva LLM 可正确求解的一个符号数学问题。虽然本例将自然语言与数学内容混合，但许多LLM使用的标准分词方式不允许这类数学输出，并且可能引发一些令人意外的问题。（图片来源：[5]，遵循知识共享许可）](assets/fig-6-7-65e2d1cc20.png)

*图：图6.7 Minerva LLM 可正确求解的一个符号数学问题。虽然本例将自然语言与数学内容混合，但许多LLM使用的标准分词方式不允许这类数学输出，并且可能引发一些令人意外的问题。（图片来源：[5]，遵循知识共享许可）*
...
![图 6.8 左上角的一个数学方程式展示了数学中出现的两种不同的表示问题。格式良好的数学需要排版语言。TeX和MathML是两种不同的排版语言，它们的文本内容截然不同，因此分词也不同。除了排版语言之外，表示同一数学语句的方式也有很多种。](assets/fig-6-8-5f352c92e9.png)
...
![图 6.9 大语言模型只有当数字分词一致时才能学会基本算术。 本图中，下划线表示不同的词元。分词后的数字可能代表任意数字的十位、百位或千位。GPT-3（左侧）对数字的分词方式不一致，使得加法运算变得不必要地复杂。GPT-4在数字的一致分词方面表现更好（](assets/fig-6-9-585e427a8b.png)
...
![图 6.11 给定某个数学目标，让LLM使用Lean（右侧路径）可能无法产生可验证的正确证明，因为它可能不擅长将Lean作为工具使用。让LLM生成常规证明（左侧路径）或许能得到正确证明，但我们无法验证其正确性。](assets/fig-6-11-c74abb647b.png)
...
![图 6.12 左侧的简化示意图展示了文本输入在进入Transformer之前如何被分词和嵌入。然后，一个解嵌入层将Transformer输出转换为所需的文本表示。当执行计算机视觉任务时，输入和输出将是图像。Transformer保持不变，但我们修改了将图](assets/fig-6-12-40b90a3ce9.png)

*图：图6.12 左侧的简化示意图展示了文本输入在进入Transformer之前如何被分词和嵌入。然后，一个解嵌入层将Transformer输出转换为所需的文本表示。当执行计算机视觉任务时，输入和输出将是图像。Transformer保持不变，但我们修改了将图像拆分为向量序列的方法：改用图像块提取而非分词。LLM使用图像块组合器生成图像输出，类似于文本LLM中的解嵌入层。*
...
![图 6.13 提取图像块是一个直接的过程。图像块提取器将图像分割成称为图像块的方形图块。图像由已经是数字的像素值组成，因此我们将每个图像块转换为数字向量。然后，我们使用一个小型神经网络作为预处理器，再将向量传递给完整的基于Transformer的神经网络](assets/fig-6-13-4312a6f875.jpg)

*图：图6.13 提取图像块是一个直接的过程。图像块提取器将图像分割成称为图像块的方形图块。图像由已经是数字的像素值组成，因此我们将每个图像块转换为数字向量。然后，我们使用一个小型神经网络作为预处理器，再将向量传递给完整的基于Transformer的神经网络。*
...
![图 6.14 与图6.13相比，此处的箭头方向相反。其目的在于强调补丁组合器与补丁提取器执行相同操作，但方向相反。在此阶段，神经网络更为重要，因为它能强制Transformer的输出与原始补丁具有相同形状——我们可以控制任何神经网络的输出尺寸。](assets/fig-6-14-68a24d9023.jpg)
...
![图 6.15 展示了四种不同的模型输入和输出组合。最右侧的例子是我们已经了解过的普通文本型LLM。向左依次是：以文本为输入的图像生成模型（“给我画一张停车标志在洪水区的图片”），以及以图像为输入、生成描述文本的图像描述模型（“这张图片显示了一个被浑浊水包](assets/fig-6-15-716206d0cd.jpg)

*图：图6.15展示了四种不同的模型输入和输出组合。最右侧的例子是我们已经了解过的普通文本型LLM。向左依次是：以文本为输入的图像生成模型（“给我画一张停车标志在洪水区的图片”），以及以图像为输入、生成描述文本的图像描述模型（“这张图片显示了一个被浑浊水包围的停车标志”）。*
...
![图 6.16 这是用旧版 Stable Diffusion（一个流行的图像生成模型）生成的。尽管告诉模型“不要画猫”，但模型被训练为生成内容。该请求超出了模型被激励去学习的范围，因此它无法处理。这与大语言模型出现接近但错误的输出问题类似，因为模型在训练期](assets/fig-6-16-2892d95c56.jpg)

*图：图 6.16 这是用旧版 Stable Diffusion（一个流行的图像生成模型）生成的。尽管告诉模型“不要画猫”，但模型被训练为生成内容。该请求超出了模型被激励去学习的范围，因此它无法处理。这与大语言模型出现接近但错误的输出问题类似，因为模型在训练期间见过类似数据。*
...
![图 7.1 与人类执行相同任务相比，LLM的优势与劣势概览。这些引出了使用LLM时必须评估的一些自然考量。据此，我们可以得出成功使用LLM的广泛建议。](assets/fig-7-1-9368a70bf8.png)

*图：图7.1 与人类执行相同任务相比，LLM的优势与劣势概览。这些引出了使用LLM时必须评估的一些自然考量。据此，我们可以得出成功使用LLM的广泛建议。*
...
![图 7.2 担忧LLMs会自我改进，需要相信LLMs不会遵循描述几乎所有其他技术发展的正常S形或收益递减S曲线。为了实现无限的自我改进，我们必须相信诸如电力、数据或计算能力等约束总是可以解决的，并且人类不会在LLM之外的领域解决这些问题。正是这些约束使](assets/fig-7-2-538647d8c6.png)

*图：图7.2 担忧LLMs会自我改进，需要相信LLMs不会遵循描述几乎所有其他技术发展的正常S形或收益递减S曲线。为了实现无限的自我改进，我们必须相信诸如电力、数据或计算能力等约束总是可以解决的，并且人类不会在LLM之外的领域解决这些问题。正是这些约束使我们能够用S曲线来描述大多数技术的发展，随着更多约束生效，进步放缓。换句话说，我们最终会达到一个无法仅仅通过建造更大的计算机来解决问题的状态。*
...
![图 7.4 包含期望LLM输出示例的提示称为少样本提示，因为LLM在训练数据中从未见过该特定行为的示例。在提示中，你可以包含与RLHF/监督式微调（SFT）类似的输入和输出示例。这种提示风格通过提供理想输出应呈现的示例，鼓励模型产生期望的输出。由于LL](assets/fig-7-4-cd2336bd64.png)
...
![图 7.5 使LLM运行所必需的昂贵硬件导致了若干权衡。例如，使用LLM的启动成本通常较高，且它们无法独立适应。这种独立适应能力的缺失导致了人类优于LLM的诸多天然弱点。部分弱点——例如模型不经训练就不会变化——反而可以被视为优势。如果每个新运行的LL](assets/fig-7-5-a3e0239fa4.png)

*图：图7.5 使LLM运行所必需的昂贵硬件导致了若干权衡。例如，使用LLM的启动成本通常较高，且它们无法独立适应。这种独立适应能力的缺失导致了人类优于LLM的诸多天然弱点。部分弱点——例如模型不经训练就不会变化——反而可以被视为优势。如果每个新运行的LLM行为都不同且不可预测，你就无法获得易于扩展的可重复流程。*
...
![图 7.6 一个人穿着或做某件不寻常事情背后的背景和原因，可能属于LLM能够正确识别并给出恰当回应的范畴。然而，如果LLM不生成一些中间文本，它可能无法得出这个恰当回应。对于数学问题，这些中间文本可能有用，但用户可能并不总是希望或适合看到这些中间文本。](assets/fig-7-6-2f3bc354d6.png)

*图：图7.6 一个人穿着或做某件不寻常事情背后的背景和原因，可能属于LLM能够正确识别并给出恰当回应的范畴。然而，如果LLM不生成一些中间文本，它可能无法得出这个恰当回应。对于数学问题，这些中间文本可能有用，但用户可能并不总是希望或适合看到这些中间文本。*
...
![图 7.7 一个计算复杂性的维恩图（假设P≠NP，对极客们来说只是个小细节）说明了这些复杂性类之间的关系。上方的箭头给出了新复杂性类所能解决的问题类型的示例，下方的箭头则显示了LLMs在复杂性层级中的位置。](assets/fig-7-7-2117b94ff2.png)

*图：图7.7 一个计算复杂性的维恩图（假设P≠NP，对极客们来说只是个小细节）说明了这些复杂性类之间的关系。上方的箭头给出了新复杂性类所能解决的问题类型的示例，下方的箭头则显示了LLMs在复杂性层级中的位置。*
...
![图 8.1 从流程图中看，用基于LLM的聊天机器人似乎可以简化和精简FAQ、邮件工单和客服电话。然而，这种观点的谬误在于流程不完整。确保LLM准确运行所需的潜在错误和补救流程被隐藏了，反而带来了更多复杂性。](assets/fig-8-1-2b55f1ea9f.png)

*图：图8.1 从流程图中看，用基于LLM的聊天机器人似乎可以简化和精简FAQ、邮件工单和客服电话。然而，这种观点的谬误在于流程不完整。确保LLM准确运行所需的潜在错误和补救流程被隐藏了，反而带来了更多复杂性。*
...
![图 8.2 一种实现“人在环路”系统的朴素方法，该系统使用LLM配合相关信息的数据库来生成输出，最终由人工工作人员审查并可能修正。](assets/fig-8-2-dfa98dbd89.png)

*图：图8.2 一种实现“人在环路”系统的朴素方法，该系统使用LLM配合相关信息的数据库来生成输出，最终由人工工作人员审查并可能修正。*
...
![图 8.3 注意，此图中的箭头方向与图8.2相比发生了变化。所有信息首先传递给人，然后我们使用大语言模型在错误发生前将其捕捉。](assets/fig-8-3-eacc87577f.png)

*图：图8.3 注意，此图中的箭头方向与图8.2相比发生了变化。所有信息首先传递给人，然后我们使用大语言模型在错误发生前将其捕捉。*
...
![图 8.4 大语言模型作为其运行的内在部分，会生成称为嵌入向量的数值向量。这些嵌入向量的实用性依赖于这样一个事实：当输入相似文本时，这些数值只发生微小变化。这里的两个示例文本将具有相似的嵌入向量，因此它们的图示看起来相似，即便它们没有任何相同的词汇。这](assets/fig-8-4-df4429d3d1.png)

*图：图8.4 大语言模型作为其运行的内在部分，会生成称为嵌入向量的数值向量。这些嵌入向量的实用性依赖于这样一个事实：当输入相似文本时，这些数值只发生微小变化。这里的两个示例文本将具有相似的嵌入向量，因此它们的图示看起来相似，即便它们没有任何相同的词汇。这是早期机器学习技术就已具备的一个强大特性。*
...
![图 8.5 此图描述了客户支持请求的“更好解决方案”：客户在等待与客服交谈时描述自己的问题。LLM 使用问题的嵌入表示来比较类似问题和已知解决方案。在用户等待期间，自动化系统可以提供有助于解决问题所需的信息，无需支持人员介入。如果这未能奏效，用户始终可](assets/fig-8-5-75da4c6271.png)

*图：图 8.5 此图描述了客户支持请求的“更好解决方案”：客户在等待与客服交谈时描述自己的问题。LLM 使用问题的嵌入表示来比较类似问题和已知解决方案。在用户等待期间，自动化系统可以提供有助于解决问题所需的信息，无需支持人员介入。如果这未能奏效，用户始终可以“退出”并与真人客服交谈。用于生成嵌入表示的模型不必与引导用户完成解决方案的 LLM 相同。*
...
![图 9.1 图9.1](assets/fig-9-1-5c597bf53b.png)
...
![图 9.2 S形曲线（Sigmoid曲线）展示了经典的平稳期行为：在某个点上，你会遇到收益递减。迭代S形曲线模型对此提出的反论点是，通过发现新技术（每个新技术用一条新的S形曲线表示），进步可以在收益递减的平稳期之后继续。新技术开始时可能比现有方法更差，](assets/fig-9-2-7545e92640.png)

*图：图9.2 S形曲线（Sigmoid曲线）展示了经典的平稳期行为：在某个点上，你会遇到收益递减。迭代S形曲线模型对此提出的反论点是，通过发现新技术（每个新技术用一条新的S形曲线表示），进步可以在收益递减的平稳期之后继续。新技术开始时可能比现有方法更差，但拥有超越它们的更大潜力。*
...
![图 9.3 图9.3](assets/fig-9-3-25e7f35edf.png)
...
![图 9.4 你可以将文本或图像视为来自数据分布，其中多样性和有趣的内容几乎必然来自分布的尾部（即分布中不那么常见的部分），因为最常见的词或内容往往是填充词或连接词，比如单词the。我们的模型不会学习它们未训练过的东西，也不能学习分布中的一切，因此从模型](assets/fig-9-4-3716765845.png)

*图：图9.4 你可以将文本或图像视为来自数据分布，其中多样性和有趣的内容几乎必然来自分布的尾部（即分布中不那么常见的部分），因为最常见的词或内容往往是填充词或连接词，比如单词the。我们的模型不会学习它们未训练过的东西，也不能学习分布中的一切，因此从模型中采样的结果必然会丢失这些有趣的细节。如果重复进行，分布会坍缩到只剩下最常见的成分。*
