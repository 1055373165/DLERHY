#!/usr/bin/env bash
# Resilient chunked translator. Logs to .scratch/translate_progress.log.
# - Pre-flight: verifies DB + API health before each batch.
# - Retries up to 3x on 5xx; 5s backoff.
set -u
DOC=d71027f0-6537-58d1-8e47-42ef2834fca4
ROOT=/Users/smy/project/book-agent/.scratch
PROGRESS="$ROOT/translate_progress.log"
IDS_FILE="$ROOT/remaining_packet_ids.json"
CHUNK=${CHUNK:-8}
...
echo "[$(date +%H:%M:%S)] starting: $COUNT packets, chunk=$CHUNK" | tee -a "$PROGRESS"
health_check || exit 1
echo "[$(date +%H:%M:%S)] health ok" | tee -a "$PROGRESS"
...
")
    ...
    while [ $ATTEMPT -lt 3 ]; do
        ...
        if [ "$HTTP" = "200" ]; then
            ...
            rm -f "$RESP_FILE"
            sleep 5
            # Re-check health between retries
            if ! health_check; then
                echo "[$(date +%H:%M:%S)] aborting due to health failure" | tee -a "$PROGRESS"
                exit 2
            fi
        fi
    done
