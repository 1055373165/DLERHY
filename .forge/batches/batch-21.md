...
Insert an explicit repair-agent adapter layer between the executor-owned `REPAIR` lane and the
specialized repair workers.

## Goals

- Let the registry resolve repair-agent adapters instead of raw workers.
- Preserve distinct review-deadlock and export-routing repair implementations behind those adapters.
- Flow adapter metadata through repair dispatch results so later remote executors can reuse the same
  contract without changing executor orchestration.
