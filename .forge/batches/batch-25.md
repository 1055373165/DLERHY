...
Frozen at: `2026-03-31 19:13:05 +0800`

## Goal

Broaden transport-backed repair execution beyond export misrouting by moving review deadlock repair
onto the same transport-backed executor plus subprocess transport path, while preserving bounded
REPAIR work-item lifecycle semantics.

## Scope

- Move review deadlock repair planning to `execution_mode=transport_backed`.
- Preserve deterministic `worker -> executor -> transport` contract routing and failure handling.
- Update acceptance fixtures so subprocess-backed repair can run against file-backed SQLite during
  review-deadlock self-heal verification.
- Keep export misrouting repair behavior intact.
