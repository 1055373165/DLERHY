...
Frozen at: `2026-03-31 20:09:47 +0800`

## Goal

Broaden transport-backed repair execution beyond local command execution by adding an HTTP repair
transport and proving a bounded repair lane can execute through a remote transport endpoint while
preserving deterministic REPAIR work-item lifecycle semantics.

## Scope

- Add `http_repair_transport` behind the existing transport registry.
- Add HTTP transport settings for endpoint, timeout, and bearer token.
- Preserve run-level transport override so review/export repair planning can select the HTTP
  transport without reopening planner/controller semantics.
- Prove export misrouting self-heal can execute through the HTTP transport path end-to-end.
