...
- mixed-layout book frontmatter and table-of-contents pages remain text-extractable but collapse into paragraph-only recovery
...
- slice-first pilot commands are directly executable from the generated pilot pack
- slice-first packet caps stop at the configured ceiling without overshooting because of extra in-flight work
- completed certified-lane pilot evidence is folded back into current readiness truth before the mainline moves on
...
5. Harden slice-first pilot control so `max_completed_packets` behaves as a strict cap under concurrency.
6. Fold completed pilot evidence into current readiness truth and use it to choose the next default move.
7. Expand the benchmark corpus before widening readiness claims further.
8. Once the first expansion-wave pair is annotated, run parser/export probes before any new translation-token pilots.
9. If an expansion-wave probe comes back `no_go`, switch immediately to parser hardening against those measured reasons instead of collecting more samples first.
10. If an expansion-wave probe comes back `go`, fold that measured result into the expansion draft and continue widening the corpus before spending new whole-document pilot tokens on uncertified lanes.
