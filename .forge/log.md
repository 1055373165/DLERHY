...
- 2026-04-03 12:35:41 +0800 | generated a certified-lane pilot pack for slice-first whole-document execution and corrected its command environment so pilot commands now run with the repo `.venv` and `PYTHONPATH=src:.`
- 2026-04-03 12:39:06 +0800 | executed the first real `L1` whole-document pilot through the normal product path; it paused legally with `pilot.slice_target_reached` after translating 15 packets and consuming `token_in 19817 / token_out 4072 / cost_usd 0.00613001`
- 2026-04-03 12:41:54 +0800 | found and fixed a slice-first control gap where `max_completed_packets` could overshoot under concurrency because in-flight work was not counted against the claim budget
- 2026-04-03 12:42:25 +0800 | revalidated the corrected slice-first control with a strict-cap smoke run; `max_completed_packets = 2` now stops exactly at `translated_packet_count = 2`
- 2026-04-03 12:43:30 +0800 | synced `.forge` and human handoff truth so the active next slice is now continue certified-lane pilots (`L2 -> L3 -> L6`) and fold pilot evidence back into readiness truth
- 2026-04-03 12:46:11 +0800 | executed the first real `L2` PDF tech-book pilot on the normal product path; it paused legally with `pilot.slice_target_reached` after translating 10 packets and consuming `token_in 13222 / token_out 2364 / cost_usd 0.00361444`
- 2026-04-03 12:48:26 +0800 | executed the first real `L3` academic-paper pilot on the normal product path; it paused legally with `pilot.slice_target_reached` after translating 8 packets and consuming `token_in 10909 / token_out 2365 / cost_usd 0.00320914`
- 2026-04-03 12:50:27 +0800 | executed the first real `L6` high-artifact-density pilot on the normal product path; it paused legally with `pilot.slice_target_reached` after translating 6 packets and consuming `token_in 8561 / token_out 2016 / cost_usd 0.00263092`
- 2026-04-03 12:52:30 +0800 | advanced translate-agent handoff truth from “begin pilots” to “first-pass pilots completed on every certified lane”; the next default slice is now fold pilot evidence back into readiness truth and choose between deeper resumes vs corpus expansion
- 2026-04-03 13:12:12 +0800 | generated a current pilot summary artifact from the four certified-lane pilot reports and their local databases, capturing usage, stop reasons, zero-failure status, and zero recent review-required hits
- 2026-04-03 13:12:12 +0800 | advanced translate-agent handoff truth so the default next slice is now benchmark corpus expansion; deeper immediate resumes on already-certified lanes are no longer the default move
- 2026-04-03 13:19:15 +0800 | generated a concrete corpus-expansion draft with six candidate samples, seeded stub gold labels, and introduced `L4` `PDF-ocr-heavy-book` plus `L5` `PDF-mixed-layout-book` as uncertified expansion lanes
- 2026-04-03 13:19:15 +0800 | advanced translate-agent handoff truth so the default next move inside corpus expansion is now annotate `pdf-building-ai-agents-001` plus one new `L4/L5` candidate before spending translation tokens on further pilot runs
- 2026-04-03 13:56:23 +0800 | promoted the first corpus-expansion pair from stub to annotated truth: `pdf-building-ai-agents-001` (`L2`) and `pdf-173140-001` (`L5`) now have concrete `annotated_v1` gold labels
- 2026-04-03 13:56:23 +0800 | generated and executed the minimal parser/export probe manifest for expansion wave 1 at `artifacts/review/translate-agent-benchmark-expansion-wave1.yaml`; both samples executed successfully with no parse failures and no translation-token spend
- 2026-04-03 13:56:23 +0800 | advanced mainline truth from “choose expansion samples” to “fix measured L2/L5 parser blockers”: wave 1 is currently `overall no_go`, with `pdf-building-ai-agents-001` failing on heading hierarchy recovery and `pdf-173140-001` failing on mixed-layout protected-artifact + heading recovery
- 2026-04-03 21:10:52 +0800 | root-caused the expansion-wave rerun hang to `_dense_toc_line_count()` on large PDF prose blocks; replaced the catastrophic regex path with a linear dense-TOC heuristic and proved both wave1 PDFs parse successfully again
- 2026-04-03 21:10:52 +0800 | hardened mixed-layout PDF recovery for wave1: preserved title-page frontmatter before TOC chapter starts, promoted cross-page inline book headings, recognized dense TOC blocks on TOC pages, and normalized repeated single-token subsection headings such as `OpenAI OpenAI`
- 2026-04-03 21:10:52 +0800 | reran `translate-agent-benchmark-expansion-wave1` end to end; current measured result is now `overall go` with `L2 = go` and `L5 = go`
- 2026-04-03 21:10:52 +0800 | advanced active handoff truth from “fix measured L2/L5 blockers” to “continue corpus expansion beyond the cleared wave1 pair”, with the next default slice aimed at first `L4` OCR-heavy probing or another low-cost EPUB family probe
