...
- A generated pilot pack is now the default execution contract for certified-lane pilots.
- First-pass pilots have now been executed on `L1`, `L2`, `L3`, and `L6`, and each paused legally via `pilot.slice_target_reached`.
- Slice-first pilot control should now be interpreted as a hard cap on `completed + inflight` packets, not merely on completed packets alone.
- Pilot evidence is now folded into current readiness truth through the current pilot summary artifact.
- The closest next slice is now `benchmark corpus expansion`, so readiness claims can extend beyond the current nine-sample set.
- Deeper immediate resumes on already-certified lanes are now optional follow-up work, not the default next move.
- The first expansion targets should be one additional mainstream PDF tech-book sample and one new uncertified lane candidate (`L4` or `L5`), not a broad unprioritized sample grab.
- The first expansion targets are now concretely measured, not just selected:
  - `pdf-building-ai-agents-001` (`L2`)
  - `pdf-173140-001` (`L5`)
- The first expansion-wave parser/export probe is now `overall go`.
- `pdf-building-ai-agents-001` (`L2`) and `pdf-173140-001` (`L5`) are both measured `go` without any translation-token pilot spend.
- Default next work returns to corpus expansion: prefer a first `L4` OCR-heavy probe or an additional EPUB family sample instead of continuing to grind already-cleared `L2/L5` parser blockers.
