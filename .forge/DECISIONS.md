...
- The next dependency-closed slice is `remote repair-agent executors`.
- Goal: keep the adapter contract deterministic, then let `worker_hint` / `worker_contract_version` route to remote or agent-backed repair executors instead of only in-process adapters.
